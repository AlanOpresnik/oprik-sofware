...
const SelectMethodToBuyService = async ({ params }: Props) => {
  const resolvedParams = await params;
  const { id } = resolvedParams;

  const plan = serviceData.find(s => s.id.toString() === id)
  if (!plan) return 'No hay plan seleccionado'
  // Datos dinámicos del plan
  const planMessage = encodeURIComponent(
    `Hola! Quiero comprar el siguiente Servicio:
...
export default SelectMethodToBuyService;
